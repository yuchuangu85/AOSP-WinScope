...
class TimeAxisPanel {
    ...
    renderCanvas(ctx, size) {
        ctx.textAlign = 'left';
        ctx.font = `11px ${css_constants_1.FONT_COMPACT}`;
    ...
    renderOffsetTimestamp(ctx) {
        ...
        switch (timestampFormat) {
            ...
            case timeline_1.TimestampFormat.Timecode:
                const width = this.renderTimestamp(ctx, timeAxisOrigin, 6, 10, gridline_helper_1.MIN_PX_PER_STEP);
                ctx.fillText('+', 6 + width + 2, 10, 6);
                break;
            case timeline_1.TimestampFormat.UTC:
                {
                    const originAsDate = time_1.Time.toDate(timeAxisOrigin, this.trace.traceInfo.unixOffset);
                    const originDate = (0, time_1.formatDate)(originAsDate, { printTime: false });
            ...
            case timeline_1.TimestampFormat.CustomTimezone:
                {
                    ...
                    const originDate = (0, time_1.formatDate)(originAsDate, {
                        tzOffsetMins,
                        printTime: false,
                    });
                    ctx.fillText(originDate, 6, 10);
...
function renderTimecode(ctx, time, x, y, minWidth) {
    ...
    ctx.font = `10.5px 10px ${css_constants_1.FONT_COMPACT}`;
