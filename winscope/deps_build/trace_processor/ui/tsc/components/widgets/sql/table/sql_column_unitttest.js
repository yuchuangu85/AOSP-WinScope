...
test('sql_column_id.basic', () => {
    ...
    expect((0, sql_column_1.sqlColumnId)('utid')).toBe('utid');
});
test('sql_column_id.single_join', () => {
    expect((0, sql_column_1.sqlColumnId)({
        column: 'bar',
        source: {
            table: 'foo',
            joinOn: {
...
test('sql_column_id.expression_without_id', () => {
    expect((0, sql_column_1.sqlColumnId)(new sql_column_1.SqlExpression((cols) => `(${cols.join('&')})`, ['a', 'b', 'c']))).toBe('a&b&c');
});
test('sql_column_id.expression_with_id', () => {
    expect((0, sql_column_1.sqlColumnId)(new sql_column_1.SqlExpression((cols) => `(${cols.join('&')})`, ['a', 'b', 'c'], 'id'))).toBe('id');
});
//# sourceMappingURL=sql_column_unitttest.js.map