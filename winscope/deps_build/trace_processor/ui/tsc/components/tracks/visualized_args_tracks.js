...
async function addVisualizedArgTracks(trace, argName) {
    ...
    for (; it.valid(); it.next()) {
        ...
        trace.tracks.registerTrack({
            uri,
            chips: ['arg'],
            renderer: await (0, visualized_args_track_1.createVisualizedArgsTrack)({
                trace,
                uri,
                trackId,
                maxDepth,
        ...
        const threadSliceTrack = trace.workspace.flatTracks.find((trackNode) => {
            ...
            return (track &&
                track.tags?.kind === track_kinds_1.SLICE_TRACK_KIND &&
                track.tags?.trackIds?.includes(trackId));
        });
        const parentGroup = threadSliceTrack?.parent;
        if (parentGroup) {
            const newTrack = new workspace_1.TrackNode({ uri, name: argName });
