...
test('TrackNode::flatTracksOrdered', () => {
    ...
    const removeme = new workspace_1.TrackNode({ uri: 'removeme' });
    root.addChildFirst(removeme);
    const foo = new workspace_1.TrackNode({ uri: 'foo' });
    root.addChildLast(foo);
    foo.addChildLast(new workspace_1.TrackNode({ uri: 'fooBar' })); // <-- Note this one is added as a child of foo
    const bar = new workspace_1.TrackNode({ uri: 'bar' });
    root.addChildLast(bar);
    root.addChildFirst(new workspace_1.TrackNode({ uri: 'baz' })); // <- Note this one is added first so should appear before the others in flatTracks
...
test('TrackNode::flatTracks', () => {
    ...
    root.addChildLast(foo);
    foo.addChildLast(new workspace_1.TrackNode({ uri: 'fooBar' })); // <-- Note this one is added as a child of foo
    root.addChildLast(new workspace_1.TrackNode({ uri: 'bar' }));
    root.addChildFirst(new workspace_1.TrackNode({ uri: 'baz' })); // <- Note this one is added first so should appear before the others in flatTracks
    root.removeChild(removeme);
    expect(root.flatTracks.map(({ uri }) => uri)).toEqual(expect.arrayContaining(['baz', 'foo', 'fooBar', 'bar']));
