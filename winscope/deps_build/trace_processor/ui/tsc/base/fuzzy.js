...
class FuzzyFinder {
    ...
    constructor(items, keyLookup) {
        this.keyLookup = keyLookup;
        // NOTE(stevegolton): This type assertion because FZF appears to be very
        // fussy about its input types.
        const options = [
            { selector: keyLookup, tiebreakers: [fzf_1.byLengthAsc] },
        ];
        this.fzf = new fzf_1.Fzf(items, ...options);
    }
    ...
    find(searchTerm) {
        return this.fzf.find(searchTerm).map((m) => {
            ...
            return {
                item: m.item,
                segments: indiciesToSegments(Array.from(m.positions).sort((a, b) => a - b), normalisedTerm),
            };
        });
    }
}
exports.FuzzyFinder = FuzzyFinder;
// Given a list of indicies of matching chars, and the original value, produce
// a list of match/nomatch segments.
function indiciesToSegments(indicies, text) {
    const segments = [];
    let nextIndex = 0;
    let match = '';
...
function match(searchTerm, text, indicies) {
...
function fuzzyMatch(text, ...searchTerms) {
    for (const searchTerm of searchTerms) {
        const indicies = new Array(searchTerm.length);
