...
describe('Time', () => {
    it('should create a new Time object with the given base and offset', () => {
        const time = new high_precision_time_1.HighPrecisionTime(t(136n), 0.3);
        expect(time.integral).toBe(136n);
        expect(time.fractional).toBeCloseTo(0.3);
    });
    it('should normalize when offset is >= 1', () => {
        let time = new high_precision_time_1.HighPrecisionTime(t(1n), 2.3);
        expect(time.integral).toBe(3n);
        expect(time.fractional).toBeCloseTo(0.3);
        time = new high_precision_time_1.HighPrecisionTime(t(1n), 1);
        expect(time.integral).toBe(2n);
        expect(time.fractional).toBeCloseTo(0);
    });
    it('should normalize when offset is < 0', () => {
        const time = new high_precision_time_1.HighPrecisionTime(t(1n), -0.4);
        expect(time.integral).toBe(0n);
        expect(time.fractional).toBeCloseTo(0.6);
    });
    it('should store timestamps without losing precision', () => {
        const time = new high_precision_time_1.HighPrecisionTime(t(1152921504606846976n));
        expect(time.toTime()).toBe(1152921504606846976n);
    });
    it('should store and manipulate timestamps without losing precision', () => {
        let time = new high_precision_time_1.HighPrecisionTime(t(2315700508990407843n));
        time = time.addTime(2315718101717517451n);
    ...
    test('equals', () => {
        const time = new high_precision_time_1.HighPrecisionTime(t(1n), 0.2);
        expect(time.equals(new high_precision_time_1.HighPrecisionTime(t(1n), 0.2))).toBeTruthy();
        expect(time.equals(new high_precision_time_1.HighPrecisionTime(t(0n), 1.2))).toBeTruthy();
        expect(time.equals(new high_precision_time_1.HighPrecisionTime(t(-100n), 101.2))).toBeTruthy();
        expect(time.equals(new high_precision_time_1.HighPrecisionTime(t(1n), 0.3))).toBeFalsy();
        expect(time.equals(new high_precision_time_1.HighPrecisionTime(t(2n), 0.2))).toBeFalsy();
    });
    ...
    test('toNumber', () => {
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.2).toNumber()).toBeCloseTo(1.2);
        expect(new high_precision_time_1.HighPrecisionTime(t(1000000000n), 0.0).toNumber()).toBeCloseTo(1e9);
    });
    test('toTime', () => {
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.2).toTime('round')).toBe(1n);
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.5).toTime('round')).toBe(2n);
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.2).toTime('floor')).toBe(1n);
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.5).toTime('floor')).toBe(1n);
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.2).toTime('ceil')).toBe(2n);
        expect(new high_precision_time_1.HighPrecisionTime(t(1n), 0.5).toTime('ceil')).toBe(2n);
    });
    test('toString', () => {
        expect(mkTime('1.3').toString()).toBe('1.3');
        expect(mkTime('12983423847.332533').toString()).toBe('12983423847.332533');
        expect(new high_precision_time_1.HighPrecisionTime(t(234n)).toString()).toBe('234');
    });
    test('abs', () => {
...
//# sourceMappingURL=high_precision_time_unittest.js.map