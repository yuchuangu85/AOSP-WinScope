...
describe('FuzzyFinder', () => {
    ...
    const finder = new fuzzy_1.FuzzyFinder(items, (x) => x);
    ...
    it('does not find completely unrelated items', () => {
        ...
        const result = finder.find('aa');
        expect(result).not.toEqual(expect.arrayContaining([expect.objectContaining({ item: 'zzz' })]));
    });
...
test('fuzzyMatch', () => {
    expect((0, fuzzy_1.fuzzyMatch)('foo bar baz', 'foo')).toEqual({
        matches: true,
        segments: [
    ...
    expect((0, fuzzy_1.fuzzyMatch)('foo bar baz', 'qux')).toEqual({
        matches: false,
        segments: [],
    });
    expect((0, fuzzy_1.fuzzyMatch)('bar baz', 'foo', 'bar')).toEqual({
        matches: true,
        segments: [
...
//# sourceMappingURL=fuzzy_unittest.js.map