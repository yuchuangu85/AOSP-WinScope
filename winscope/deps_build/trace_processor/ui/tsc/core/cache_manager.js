...
async function cacheDelete(key) {
    try {
        ...
        if (cache === undefined)
            return false; // Cache storage not supported.
...
async function cachePut(key, value) {
    try {
        const cache = await getCache();
        if (cache === undefined)
            return; // Cache storage not supported.
...
async function cacheMatch(key) {
    try {
        const cache = await getCache();
        if (cache === undefined)
            return undefined; // Cache storage not supported.
...
async function cacheTrace(traceSource, traceUuid) {
    let trace;
    let title = '';
    let fileName = '';
    let url = '';
