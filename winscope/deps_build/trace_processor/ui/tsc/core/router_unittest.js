...
describe('Router#resolve', () => {
    ...
    const pluginId = plugin_manager_1.CORE_PLUGIN_ID;
    const traceless = true;
    test('Resolves empty route to default component', () => {
        const pages = new page_manager_1.PageManagerImpl();
    ...
    test('Resolves subpage route to component of main page', () => {
        ...
        expect(pages.renderPageForCurrentRoute(undefined).tag).toBe(nonDefaultComponent);
        expect(pages.renderPageForCurrentRoute(undefined).attrs.subpage).toBe('/subpage');
    });
...
describe('Router.parseUrl', () => {
    ...
    test('Search parsing', () => {
        ...
        const route = router_1.Router.parseUrl(url);
    ...
    test('Fragment parsing', () => {
        const url = 'http://localhost/?p=1&s=2&hideSidebar=true#!/foo?s=3&url=4&hideSidebar=false';
        const route = router_1.Router.parseUrl(url);
        const args = route.args;
    ...
    test('Nested fragment', () => {
        const url = 'http://localhost/?p=1&s=2&hideSidebar=true#!/foo?s=3&url=4&hideSidebar=false#myfragment';
        const route = router_1.Router.parseUrl(url);
...
describe('Router.parseFragment', () => {
    test('empty route broken into empty components', () => {
        const { page, subpage, args } = router_1.Router.parseFragment('');
        expect(page).toBe('');
        expect(subpage).toBe('');
    ...
    test('by default args are undefined', () => {
        ...
        const { args } = router_1.Router.parseFragment('');
        expect(args).toEqual({});
    });
    test('invalid route broken into empty components', () => {
        const { page, subpage } = router_1.Router.parseFragment('/bla');
        expect(page).toBe('');
        expect(subpage).toBe('');
    });
    test('simple route has page defined', () => {
        const { page, subpage } = router_1.Router.parseFragment('#!/record');
        expect(page).toBe('/record');
        expect(subpage).toBe('');
    });
    test('simple route has both components defined', () => {
        const { page, subpage } = router_1.Router.parseFragment('#!/record/memory');
        expect(page).toBe('/record');
        expect(subpage).toBe('/memory');
    });
    test('route broken at first slash', () => {
        const { page, subpage } = router_1.Router.parseFragment('#!/record/memory/stuff');
        expect(page).toBe('/record');
        expect(subpage).toBe('/memory/stuff');
    });
    test('parameters separated from route', () => {
        const { page, subpage, args } = router_1.Router.parseFragment('#!/record/memory?url=http://localhost:1234/aaaa');
        expect(page).toBe('/record');
        expect(subpage).toBe('/memory');
        expect(args.url).toEqual('http://localhost:1234/aaaa');
    });
    test('openFromAndroidBugTool can be false', () => {
        const { args } = router_1.Router.parseFragment('#!/?openFromAndroidBugTool=false');
        expect(args.openFromAndroidBugTool).toEqual(false);
    });
    test('openFromAndroidBugTool can be true', () => {
        const { args } = router_1.Router.parseFragment('#!/?openFromAndroidBugTool=true');
        expect(args.openFromAndroidBugTool).toEqual(true);
    });
    test('bad modes are coerced to default', () => {
        const { args } = router_1.Router.parseFragment('#!/?mode=1234');
        expect(args.mode).toEqual(undefined);
    });
    test('bad hideSidebar is coerced to default', () => {
        const { args } = router_1.Router.parseFragment('#!/?hideSidebar=helloworld!');
        expect(args.hideSidebar).toEqual(undefined);
    });
});
//# sourceMappingURL=router_unittest.js.map