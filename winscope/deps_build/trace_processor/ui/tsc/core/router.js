...
const route_schema_1 = require("../public/route_schema");
...
//   this.
function safeParseRoute(rawRoute) {
    const res = route_schema_1.ROUTE_SCHEMA.safeParse(rawRoute);
...
class Router {
    ...
    static parseFragment(hash) {
        if (hash.startsWith(exports.ROUTE_PREFIX)) {
            hash = hash.substring(exports.ROUTE_PREFIX.length);
        }
        else {
            hash = '';
        }
    ...
    static parseQueryString(query) {
        ...
        return mithril_1.default.parseQueryString(query);
    }
