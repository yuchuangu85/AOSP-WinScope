...
class default_1 {
    static id = 'dev.perfetto.AndroidNetwork';
    ...
    async addSimpleTrack(ctx, trackName, tableOrQuery, columns) {
    ...
    async onTraceLoad(ctx) {
        ctx.commands.registerCommand({
            ...
            callback: async (track) => {
                ...
                await this.addSimpleTrack(ctx, track, `(SELECT *
            FROM android_battery_stats_event_slices
            WHERE track_name = "${track}")`, ['ts', 'dur', 'str_value', 'int_value']);
            },
        });
        ctx.commands.registerCommand({
        ...
        `);
                ...
                await this.addSimpleTrack(ctx, trackName ?? 'Network Activity', `android_network_activity_${suffix}`, ['ts', 'dur', ...groupCols, 'packet_length', 'packet_count']);
            },
        });
    }
}
