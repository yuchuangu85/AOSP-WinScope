...
exports.memoryRecordSection = memoryRecordSection;
...
function heapProfiling() {
    const settings = {
        targetProcs: new textarea_1.Textarea({
        ...
        samplingBytes: new slider_1.Slider({
        ...
        dumpInterval: new slider_1.Slider({
    ...
    return {
        ...
        description: 'Track native heap allocations & deallocations of an Android ' +
        ...
        genConfig: function (tc) {
            ...
            tc.addDataSource('android.heapprofd').heapprofdConfig = {
                ...
                continuousDumpConfig: s.dumpInterval.value == 0
                    ? undefined
                    : {
...
function heapDumps() {
    ...
    return {
        ...
        description: 'Dump information about the Java object graph of an ' +
...
function meminfo() {
    const meminfoCounters = new Map();
    for (const x in protos_1.default.MeminfoCounters) {
        if (typeof protos_1.default.MeminfoCounters[x] === 'number' &&
            !`${x}`.endsWith('_UNSPECIFIED')) {
            meminfoCounters.set(x.replace('MEMINFO_', '').toLowerCase(), protos_1.default.MeminfoCounters[x]);
        }
    }
    const settings = {
        pollMs: new slider_1.Slider(slider_1.POLL_INTERVAL_SLIDER),
        counters: new multiselect_1.TypedMultiselect({
...
function vmstat() {
    const vmstatCounters = new Map();
    for (const x in protos_1.default.VmstatCounters) {
        if (typeof protos_1.default.VmstatCounters[x] === 'number' &&
            !`${x}`.endsWith('_UNSPECIFIED')) {
            vmstatCounters.set(x.replace('VMSTAT_', '').toLowerCase(), protos_1.default.VmstatCounters[x]);
        }
    }
    const settings = {
        pollMs: new slider_1.Slider(slider_1.POLL_INTERVAL_SLIDER),
    ...
    return {
        ...
        description: 'Periodically polls virtual memory stats from /proc/vmstat. ' +
...
function hifreq() {
    return {
        ...
        description: 'Allows to track short memory spikes and transitories through ' +
            "ftrace's mm_event, rss_stat and ion events. Available only " +
            'on recent Android Q+ kernels',
        supportedPlatforms: ['ANDROID'],
        genConfig: function (tc) {
            tc.addFtraceEvents('mm_event/mm_event_record', 'kmem/rss_stat', 'ion/ion_stat', 'dmabuf_heap/dma_heap_stat', 'kmem/ion_heap_grow', 'kmem/ion_heap_shrink');
        },
    };
}
function lmk() {
    return {
        ...
        image: 'rec_lmk.png',
        dependencies: [advanced_1.ADV_PROC_ASSOC_PROBE_ID],
        description: 'Record LMK events. Works both with the old in-kernel LMK ' +
        ...
        genConfig: function (tc) {
            ...
            'lowmemorykiller/lowmemory_kill', 'oom/oom_score_adj_update');
...
function polledProcStats() {
    const settings = {
        pollMs: new slider_1.Slider(slider_1.POLL_INTERVAL_SLIDER),
        procAge: new toggle_1.Toggle({ title: 'Record process age' }),
        procRuntime: new toggle_1.Toggle({ title: 'Record process runtime' }),
    };
    return {
        id: 'mem_proc_stat',
...
function extractCmdlinesAndPids(text) {
    const cmdlines = [];
    const pids = [];
    for (const line of (0, string_utils_1.splitLinesNonEmpty)(text)) {
