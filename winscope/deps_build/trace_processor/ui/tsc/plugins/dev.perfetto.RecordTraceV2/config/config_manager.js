...
const config_interfaces_1 = require("./config_interfaces");
const trace_config_builder_1 = require("./trace_config_builder");
/**
...
class ConfigManager {
    probesById = new Map();
    _traceConfig = new trace_config_builder_1.TraceConfigBuilder();
    enabledProbes = new Map();
    indirectlyEnabledProbes = new Map();
    get traceConfig() {
        return this._traceConfig;
    }
    registerProbes(probes) {
        for (const probe of probes) {
            (0, logging_1.assertFalse)(this.probesById.has(probe.id));
            this.probesById.set(probe.id, probe);
        }
    }
    setProbeEnabled(probeId, enabled) {
        const probe = (0, logging_1.assertExists)(this.probesById.get(probeId));
        this.enabledProbes.set(probeId, enabled);
        for (const depProbeId of probe.dependencies ?? []) {
            (0, logging_1.assertTrue)(this.probesById.has(depProbeId));
            const depSet = (0, utils_1.getOrCreate)(this.indirectlyEnabledProbes, depProbeId, () => new Set());
            if (enabled) {
                depSet.add(probeId);
            }
            else {
    ...
    isProbeEnabled(probeId) {
        const directlyEnabled = this.enabledProbes.get(probeId) === true;
        const enabledDueToDeps = Boolean(this.indirectlyEnabledProbes.get(probeId)?.size);
     ...
     */
    getProbeEnableDependants(probeId) {
        return Array.from(this.indirectlyEnabledProbes.get(probeId) ?? []).map((id) => (0, logging_1.assertExists)(this.probesById.get(id)).title);
    }
    /**
     * Generates the TraceConfig proto for the current configuration.
     */
    genTraceConfig(platform) {
        ...
        for (const bufId of this._traceConfig.buffers.keys()) {
            if (bufId !== trace_config_builder_1.DEFAULT_BUFFER_ID) {
        ...
        for (const probe of orderedProbes) {
            if (!(0, config_interfaces_1.supportsPlatform)(probe, platform))
                continue;
    ...
    serializeProbes() {
        return Object.fromEntries(this.getProbesOrderedByDep(/* enabledOnly */ true).map((probe) => [
            probe.id,
            {
                settings: Object.fromEntries(Object.entries(probe.settings ?? {}).map(([settingId, setting]) => [
                    settingId,
                    setting.serialize(),
                ])),
            },
    ...
    getProbesOrderedByDep(enabledOnly = false) {
        const orderedProbes = [];
        const seenIds = new Set();
        const queueProbe = (probeId) => {
            if (enabledOnly && !this.isProbeEnabled(probeId))
                return;
            const probe = (0, logging_1.assertExists)(this.probesById.get(probeId));
            if (orderedProbes.includes(probe))
