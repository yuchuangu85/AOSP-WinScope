...
(0, test_1.test)('multiple traces via url and local_cache_key', async ({ browser }) => {
    const page = await browser.newPage();
    const pth = new perfetto_ui_test_helper_1.PerfettoTestHelper(page);
    // Open first trace.
    await pth.navigate('#!/?url=http://127.0.0.1:10000/test/data/perf_sample_annotations.pftrace');
    const cacheKey1 = page.url().match(/local_cache_key=([a-z0-9-]+)/)[1];
    await pth.waitForIdleAndScreenshot('trace_1.png');
    // Open second trace.
    await pth.navigate('#!/?url=http://127.0.0.1:10000/test/data/atrace_compressed.ctrace');
    const cacheKey2 = page.url().match(/local_cache_key=([a-z0-9-]+)/)[1];
    (0, test_1.expect)(cacheKey1).not.toEqual(cacheKey2);
...
//# sourceMappingURL=local_cache_key.test.js.map