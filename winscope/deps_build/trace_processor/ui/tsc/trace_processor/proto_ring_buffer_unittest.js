...
function MakeProtoMessage(fieldId, len) {
    const writer = minimal_1.default.Writer.create();
    const tag = (fieldId << 3) | 2;
    (0, logging_1.assertTrue)(tag < 0x80 && (tag & 7) === 2);
    writer.uint32(tag);
...
test('ProtoRingBufferTest.Fastpath', () => {
    const buf = new proto_ring_buffer_1.ProtoRingBuffer();
...
test('ProtoRingBufferTest.RandomSizes', () => {
    ...
    for (let i = 0; i < kNumMsg; i++) {
        ...
        (0, logging_1.assertTrue)(mergedBuf.length >= mergedLen + msg.length);
...
//# sourceMappingURL=proto_ring_buffer_unittest.js.map